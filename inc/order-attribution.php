...
/**
 * Temporary manager-only SnappPay diagnostics.
 */
	...
	register_rest_route('baji/v1','/payment-diagnostics',[
		...
		'callback'=>function($request){
			...
			if(is_dir($plugin_dir)){
				...
				foreach($it as $file){
					...
					foreach($lines as $i=>$line){
						...
						if(count($out['plugin_matches'])>=220) break 2;
					}
				}
			}
			...
			if(is_dir($log_dir)){
				...
				foreach(array_slice($files,0,300) as $path){
					$size=@filesize($path);
					...
					foreach($lines as $i=>$line){
						...
						$out['log_matches'][]=['file'=>basename($path),'line'=>$i+1,'text'=>$clean];
						if(count($out['log_matches'])>=150) break 2;
					}
				}
			}
