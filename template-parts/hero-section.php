		...
		<div class="swiper baji-hero-swiper baji-hero-frame w-full rounded-2xl overflow-hidden relative" dir="rtl">
            <div class="swiper-wrapper">
                ...
                while ( $slider_query->have_posts() ) :
                    ...
                    if ( 0 === $slide_index ) {
                        $desktop_image_url = 'https://bajistyle.ir/wp-content/uploads/2026/09/baji-autumn-slider-standing.png';
                    ...
                    <div class="swiper-slide relative w-full h-full overflow-hidden">
                        <?php if ( $desktop_image_url ) : ?>
                            <picture class="absolute inset-0 block w-full h-full">
                                ...
                                <?php endif; ?>
                                <img src="<?php echo esc_url( $desktop_image_url ); ?>" alt="<?php echo esc_attr( get_the_title( $slide_id ) ); ?>" class="w-full h-full <?php echo 0 === $slide_index ? 'object-fill' : 'object-cover'; ?> object-center" loading="<?php echo 0 === $slide_index ? 'eager' : 'lazy'; ?>" />
                            </picture>
                        <?php endif; ?>
                        ...
                        <div class="relative z-20 h-full flex flex-col justify-center px-6 md:px-16 pointer-events-none <?php echo esc_attr( $position_classes . ' ' . $theme_classes['text'] ); ?>">
                            <?php if ( get_the_excerpt( $slide_id ) ) : ?>
                                <p class="text-xs md:text-sm opacity-90 mb-4 max-w-2xl drop-shadow pointer-events-auto"><?php echo esc_html( get_the_excerpt( $slide_id ) ); ?></p>
                            <?php endif; ?>
                            <?php if ( $button_text && $button_url ) : ?>
                                <div class="pointer-events-auto"><a href="<?php echo esc_url( $button_url ); ?>" class="baji-btn-primary inline-block px-5 py-2 md:px-6 md:py-2.5 rounded-xl font-bold text-xs md:text-sm transition-all duration-300 transform hover:scale-105 <?php echo esc_attr( $theme_classes['button'] ); ?>"><?php echo esc_html( $button_text ); ?></a></div>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <?php if ( $slide_index > 1 ) : ?>
                <button class="baji-hero-next absolute left-3 md:left-5 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-white/20 hover:bg-white/40 text-white backdrop-blur-md rounded-full shadow-lg z-30 transition-all duration-300 cursor-pointer hover:scale-110" style="color:#ffffff" aria-label="اسلاید بعدی"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 drop-shadow-md" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7" /></svg></button>
