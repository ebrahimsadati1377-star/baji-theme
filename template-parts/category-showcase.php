...
.baji-category-showcase{padding:22px 0 28px;background:#fff}
.baji-category-showcase__head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:16px}
.baji-category-showcase__title{margin:0;font-size:18px;font-weight:900;color:#211b1b;letter-spacing:-.02em}
.baji-category-showcase__hint{font-size:11px;color:#8c7b78}
.baji-category-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
.baji-category-card{position:relative;display:block;overflow:hidden;border-radius:15px;background:#f3e4da;aspect-ratio:1.85/1;text-decoration:none!important;box-shadow:0 3px 12px rgba(61,35,31,.07);isolation:isolate;transition:transform .2s ease,box-shadow .2s ease}
.baji-category-card:active{transform:scale(.985)}
.baji-category-card__image{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;transition:transform .35s ease}
.baji-category-card:hover .baji-category-card__image{transform:scale(1.035)}
.baji-category-card__shade{position:absolute;inset:0;background:linear-gradient(90deg,rgba(31,20,18,.56) 0%,rgba(31,20,18,.27) 48%,rgba(31,20,18,.03) 100%);z-index:1}
...
?>
<a href="<?php echo esc_url($category_link); ?>" class="baji-category-card" role="listitem" aria-label="<?php echo esc_attr(sprintf(__('مشاهده دسته %s','bajistyle'),$category->name)); ?>">
...
<span class="baji-category-card__shade" aria-hidden="true"></span>
<span class="baji-category-card__content"><strong class="baji-category-card__name"><?php echo esc_html($category->name); ?></strong><i class="fa-solid fa-arrow-left baji-category-card__arrow" aria-hidden="true"></i></span>
</a>
<?php endforeach; ?>
</div>
</div>
