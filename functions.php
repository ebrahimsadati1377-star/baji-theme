...
/* Temporary admin-only Digikala checkout visibility diagnostic. */
function baji_debug_digikala_visibility() {
	$out = array();
	if ( function_exists( 'WC' ) && WC() && WC()->payment_gateways() ) {
		...
		if ( isset( $gws['bwdk'] ) ) {
			...
			$out['gateway'] = array(
				...
				'title' => wp_strip_all_tags( (string) ( $g->title ?? '' ) ),
				'is_available' => method_exists( $g, 'is_available' ) ? (bool) $g->is_available() : null,
				'supports' => isset( $g->supports ) ? array_values( (array) $g->supports ) : array(),
			);
			try {
				$r = new ReflectionClass( $g );
				$file = $r->getFileName();
				$out['gateway']['file'] = $file ? str_replace( ABSPATH, '', $file ) : '';
				if ( $file && is_readable( $file ) ) {
					$lines = file( $file );
					$start = max( 0, $r->getStartLine() - 1 );
					$len = min( 260, max( 1, $r->getEndLine() - $r->getStartLine() + 1 ) );
					$out['source'] = implode( '', array_slice( $lines, $start, $len ) );
				}
			} catch ( Throwable $e ) {
	...
	$out['options'] = get_option( 'woocommerce_bwdk_settings', array() );
	if ( is_array( $out['options'] ) ) {
		foreach ( $out['options'] as $k => $v ) {
			if ( preg_match('/token|secret|password|key|credential/i', (string) $k ) ) {
				$out['options'][ $k ] = empty( $v ) ? '' : '[SET]';
			}
		}
	}
	return $out;
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'baji-debug/v1', '/digikala-visibility', array(
		'methods' => 'GET',
		'permission_callback' => function () { return current_user_can( 'manage_options' ); },
		'callback' => function () { return rest_ensure_response( baji_debug_digikala_visibility() ); },
	) );
} );
