 ...
 * ثابت‌های قالب
 * ---------------------------------------------------------------------- */
define( 'BAJISTYLE_VERSION', '1.0.57' );
...
function bajistyle_enqueue_assets() {
	...
	wp_enqueue_script(
		...
		array( 'swiper-js', 'bajistyle-navigation', 'bajistyle-animations' ),
		BAJISTYLE_VERSION,
		array(
			'in_footer' => true,
			'strategy'  => 'defer',
	...
	if ( class_exists( 'WooCommerce' ) ) {
		...
		if ( is_shop() || is_product_category() || is_product_tag() ) {
			...
			wp_enqueue_script(
				'bajistyle-shop-page',
				BAJISTYLE_URI . '/assets/js/shop-page.js',
				array(),
				BAJISTYLE_VERSION,
				array(
					'in_footer' => true,
		...
		if ( function_exists( 'is_account_page' ) && is_account_page() ) {
			wp_enqueue_style(
				'bajistyle-account-style',
				BAJISTYLE_URI . '/assets/css/account.css',
				array( 'bajistyle-custom' ),
				BAJISTYLE_VERSION
			);
		}
...
/**
 * One-time LiteSpeed purge after related-product regular-price styling update.
 */
function baji_purge_litespeed_related_price_1057() {
    $key = 'baji_lscache_purge_related_price_1057';
    if ( 'done' === get_option( $key ) ) {
        return;
    }
    if ( has_action( 'litespeed_purge_all' ) || defined( 'LSCWP_V' ) ) {
